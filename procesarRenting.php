<?php
    ...
    function insertarReservaCoche($conn, $marca, $modelo, $nombre, $apellidos, $carnet, $precio, $telefono) {
    try {
        $sql = "INSERT INTO reservasCarRental (matricula,marca, modelo, nombreUsuario, nombre, apellidos, carnet, precio, telefono) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";       
        $stmt = $conn->prepare($sql);
        // Ejecutar la inserción
        if ($stmt->execute(['', $marca, $modelo, $_SESSION['usuario'], $nombre, $apellidos, $carnet, $precio, $telefono])) {
            return true;
        } else {
            return false;
        }
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        if(empty($carnet['name'])){
            $errores['carnet'] = "<h3 style='color:red'>Este campo no puede estar vacío</h3>";
        } else {
            if($carnet['type'] != 'application/pdf' && $carnet['type'] != 'image/png'){
                $errores['carnet'] = "<h3 style='color:red'>El formato debe ser válido (PDF o PNG)</h3>";
            }
        }
        ...
        if(empty($errores)){
            $resultado = insertarReservaCoche($conn, $marca, $modelo, $dni, $nombre, $apellidos, $gotIt, $precioFinal, $numTlf);
            if ($resultado) {
...
<body>
    ...
    <main>
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-label">
                <label>Carnet de conducir (Imagen/PDF)</label>
